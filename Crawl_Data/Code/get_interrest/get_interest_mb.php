...
define('BASE_URL','https://www.mbbank.com.vn/');
...
$root = 'https://beta.mbbank.com.vn/congcu/Lists/LaiSuat/lai-suat.aspx';
...
function save_all_post($html){
    ...
    $block = $dom->find('div[id=WebPartWPQ12]')[0];

    $newsItems = $block->find('tr[class=lv-itmhover-laisuat]');
    
    if(isset($newsItems) && count($newsItems) > 1){
            for($i = 1; $i < count($newsItems); ++$i){
                $newsItem = $newsItems[$i];
                //echo $newsItems[$i];
            $title = $newsItem->find('td[class^=lv-vb2]')->text();
...
function get_content($link){
    if(get_data($link, $content)){
         ...
         $html = $dom->find('div[id=WebPartWPQ12]')[0]->html();
         return $html; 
    }
